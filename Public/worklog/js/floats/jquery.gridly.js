...
(function() {
  "use strict";
  var $, Animation, Draggable, Gridly, ChildBlock,
  ...
  Draggable = (function() {
    ...
    //浮动模块子模块点击
    Draggable.prototype.child_block = function(event) {
      //判断点击位置
      var notclose=0;
      //判断是否选中项目
      var pid;
      var noclass=0;
      pid=$("#logs li:eq(1)").data("pid");
      $("#project-details-pid").val(pid);
      $("#logs li").each(function(){
        if(pid!=$(this).data("pid")){
          noclass=1;//没有项目
        }
      })
      if(this.$target.context.className.indexOf('statistical')>=0){
        notclose=1;
        location.href=controller+"/statistical";
      }
      //打开项目描述页面
      if(this.$target.context.className.indexOf('details')>=0){
        notclose=1;
        if(noclass==0){
           this.get_details();
        }
      ...
      //筛选BUG
      if(this.$target.context.className.indexOf('bug')>=0){
        notclose=1;
        if(noclass==0){
          this.get_bug();
        }
      ...
      //筛选BUG
      if(this.$target.context.className.indexOf('function')>=0){
        notclose=1;
        if(noclass==0){
          this.get_function();
        }
      ...
      if(this.move==1){
        ...
        var move_box=$(".extend-box");
        if(move_box.hasClass("active")&&notclose==0){
          move_box.removeClass("active")
          $(".statistical").animate({"left":"0"},200);
          $(".details").animate({"top":"0"},200);
          $(".bug").animate({"top":"0"},200);
          $(".function").animate({"left":"0"},200);
        }else{
          move_box.addClass("active")
          $(".statistical").animate({"left":"10vw"},200);
          $(".details").animate({"top":"10vw"},200);
          $(".bug").animate({"top":"-10vw"},200);
          $(".function").animate({"left":"-10vw"},200);
        }
        console.log("点击");
      }
    }
    ...
    //获取项目描述
    Draggable.prototype.get_details = function(event) {
      var pid = $("#project-details-pid").val();
      $.ajax({
        url:controller+"/getProjectDescribe",
        data:{pid:pid},
        dataType:'json',
        success:function(data){
          console.log(data);
          $("#o-content").val(data.data);
        }
      })
      $("#details-wrap").toggle();
    }
    //筛选BUG项目
    Draggable.prototype.get_bug = function(event) {
      var pid = $("#project-details-pid").val();
      $("#project-details-pid").val(pid);
      $.ajax({
        url:controller+"/getLogsType",
        data:{pid:pid,type:2},
        dataType:'json',
        success:function(data){
          console.log(data);
    ...
    //筛选功能项目
    Draggable.prototype.get_function = function(event) {
      var pid = $("#project-details-pid").val();
      $("#project-details-pid").val(pid);
      $.ajax({
        url:controller+"/getLogsType",
    ...
    Draggable.prototype.moved = function(event) {
      ...
      event.preventDefault();
      event.stopPropagation();
      var moveX=this.coordinate(event).pageX - this.origin.x;
      var moveY=this.coordinate(event).pageY - this.origin.y;
      this.$target.css({
        left: moveX,
        top: moveY
      });
